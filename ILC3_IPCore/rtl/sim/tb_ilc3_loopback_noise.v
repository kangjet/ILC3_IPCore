...
//======================================================
// ILC3 0-code TX/RX Loopback Testbench (NOISE / ISI VARIANT)
//
// Purpose:
//   - Verify basic TX/RX datapath of ILC3 0-code implementation.
//   - Fixed pattern 0,1,2,3,... under a channel with optional noise/ISI.
//   - Check 1-symbol pipeline latency in RX and scoreboard matching under non-ideal conditions.
//
// Channel configuration:
//   - ilc3_channel_model with (default example):
//       AMP_WIDTH = 4
//       GAIN_NUM  = 1
//       GAIN_DEN  = 1
//       OFFSET    = 0
//       ADD_NOISE = 2  (enable internal noise model; see ilc3_channel_model.v)
//
// Test configuration:
//   - N_SYM = 1024 symbols.
//   - TX pattern: 0,1,2,3,0,1,2,3,... (exp_sym[i] = i[1:0])
//   - Scoreboard compares RX symbol at rx_idx with TX[rx_idx-1]
//     because RX has 1-symbol pipeline latency.
//
// How to run (example):
//   iverilog -g2012 -o ilc3_loop_noise \
//     ../core/ilc3_tx_core.v \
//     ../core/ilc3_rx_core.v \
//     ilc3_channel_model.v \
//     tb_ilc3_loopback_noise.v
//
//   vvp ilc3_loop_noise
//
// Expected:
//   - Depending on ADD_NOISE/ISI settings, err_cnt may be 0 (very clean) or non-zero (to observe BER under noise).
//
//   This testbench is intended for non-ideal channel checks (noise/ISI)
//   and for early BER-style experiments at RTL level (before full PHY).
//======================================================

//======================================================
// Top-level testbench module
//======================================================
module tb_ilc3_loopback_noise;
    ...
    //==================================================
    // Channel configuration parameters (noise / ISI variant)
    //   - You can tweak these for different RTL-level experiments.
    //==================================================
    localparam integer CH_AMP_WIDTH = 4;
    localparam integer CH_GAIN_NUM  = 1;
    localparam integer CH_GAIN_DEN  = 1;
    localparam integer CH_OFFSET    = 0;

    // ADD_NOISE: 0 = no noise, >0 = enable internal noise model (see ilc3_channel_model.v)
    // CH_ADD_NOISE를 컴파일 옵션(-DCH_ADD_NOISE=1 등)으로 덮어쓸 수 있도록 매크로를 사용.
    `ifndef CH_ADD_NOISE
      `define CH_ADD_NOISE 2
    `endif
    localparam integer CH_ADD_NOISE = `CH_ADD_NOISE;  // 기본값 2, -DCH_ADD_NOISE=1 등으로 변경 가능

    //==================================================
    // Loopback path: TX -> Channel model -> RX
    // - Channel here is configured for noise/ISI experiments.
    //   By default (CH_ADD_NOISE=2) a built-in noise model is enabled.
    //   You can sweep CH_ADD_NOISE/GAIN/OFFSET to emulate different channels.
    //==================================================
    ilc3_channel_model #(
        .AMP_WIDTH(CH_AMP_WIDTH),
        .GAIN_NUM (CH_GAIN_NUM ),
        .GAIN_DEN (CH_GAIN_DEN ),
        .OFFSET   (CH_OFFSET   ),
        .ADD_NOISE(CH_ADD_NOISE)
    ) u_ch (
        .clk          (clk),
        .rst_n        (rst_n),
        .amp_in       (amp_out),
    ...
    //==================================================
    // 기본 심볼 개수: 1024, 필요 시 -DTB_N_SYM=10000 식으로 오버라이드
    `ifndef TB_N_SYM
