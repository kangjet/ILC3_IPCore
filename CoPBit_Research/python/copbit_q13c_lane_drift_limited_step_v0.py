#!/usr/bin/env python3
"""
CoPBit Q13c – lane & drift sweep with limited-step Kuramoto:
xN lanes CoPBit-M8(8-PSK) + Channel-b + FFE + limited-step Kuramoto (Eb/N0 basis) v0.1
...
- 를 쓸 때,
  ...
  · BER_kura (Kuramoto ON, limited-step)
  변화를 보는 것.

변경 포인트(Q13b → Q13c):
- Kuramoto 위상 업데이트를 limited-step 방식으로 변경:
    err_phasor = mean(z_n * conj(s_hat))
    err       = angle(err_phasor)
    delta     = mu_phase * err
    delta     = clip(delta, -phi_max_rad, +phi_max_rad)
    phi_est  += delta
- 한 심볼에서 추정 위상이 과도하게 튀지 않도록 φ_max_deg를 브레이크로 사용.
"""
...
# -------------------------- xN lanes + EQ + limited-step Kuramoto --------------------------

def simulate_m8_xn_eq_kuramoto_ber_limited(
    ebn0_db,
    n_sym,
    n_lanes,
    channel_taps,
    ...
    글로벌 위상 드리프트 + limited-step Kuramoto 위상 추적

    반환:
    - ber_base: Kuramoto 없이, 각 레인을 독립 8-PSK로 디코딩한 BER
    - ber_kura: limited-step Kuramoto 위상 추적 후 디코딩한 BER
    """
    kbits = 3
    ...
    # 7) limited-step Kuramoto 기반 글로벌 위상 추적 + 디코딩
    phi_est = 0.0
...
def parse_args():
    p = argparse.ArgumentParser(
        description="CoPBit Q13c – lane & drift sweep for xN lanes M8 + Channel-b + FFE + limited-step Kuramoto (Eb/N0 basis) v0.1"
    )
...
def main():
    ...
    print("=== CoPBit Q13c – lane & drift sweep for xN lanes M8 + Channel-b + FFE + limited-step Kuramoto (Eb/N0 basis) v0.1 ===")
    ...
    for drift_std_deg in drift_list:
        ...
        print(" n_lanes | EbN0_dB | BER_M8_base | BER_M8_kura(limited)")
        print("--------------------------------------------------------")

        for n_lanes in lanes_list:
            for ebn0_db in ebn0_list:
                ber_base, ber_kura = simulate_m8_xn_eq_kuramoto_ber_limited(
                    ebn0_db=ebn0_db,
                    n_sym=args.n_sym,
                    n_lanes=n_lanes,
                    channel_taps=h_b,
    ...
    print(" - Q13b 대비 변경점:")
    print("     · Kuramoto 위상 업데이트에 1-step 최대 변화량 phi_max_deg(deg)를 도입.")
    print("     · 과도한 err에 의해 위상 추정이 발산하는 것을 억제하는 브레이크 역할.")
    print(" - lane 수(L)를 늘리면 Kuramoto에서 사용하는 평균 phasor의 잡음이 줄어들어,")
    print("   특히 drift_std_deg ≲ 1° 영역에서 limited-step과 결합 시")
    print("   더 안정된 위상 락 특성을 기대할 수 있다.")
    print(" - drift_std_deg가 2° 이상 커지는 경우에도")
    print("   Q13b 대비 어느 정도 버텨주는지(또는 거의 동일하게 망가지는지)를 비교해서,")
    print("   CoPBit 멀티레인 위상 락의 실제 허용 드리프트 범위를 정교하게 좁히는 용도로 쓸 수 있다.")
