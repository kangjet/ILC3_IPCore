#!/usr/bin/env python3
"""
CoPBit Q13b – lane 수 & 드리프트 스윕:
xN lanes CoPBit-M8(8-PSK) + Channel-b + FFE + Kuramoto (Eb/N0 basis) v0.1
...
- 를 쓸 때,
  lane 수(n_lanes)와 drift_std_deg에 따라 BER_base / BER_kura가 어떻게 변하는지 맵을 보는 것.

주의:
- 채널은 실수 5-tap FIR (b 채널): [0.05, 0.5, 1.0, 0.5, 0.05]
- FFE는 심볼 레이트 LS 기반, 길이 L=ffe_len, train_frac 비율만큼의 심볼로 설계
- PAM4는 여기서는 다루지 않고, M8(CoPBit 위상)만 lane/드리프트 민감도 분석용으로 사용.
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="CoPBit Q13b – lane & drift sweep for xN lanes M8 + Channel-b + FFE + Kuramoto (Eb/N0 basis) v0.1"
    )
...
def main():
    ...
    print("=== CoPBit Q13b – lane & drift sweep for xN lanes M8 + Channel-b + FFE + Kuramoto (Eb/N0 basis) v0.1 ===")
    ...
    for drift_std_deg in drift_list:
        ...
        print(" n_lanes | EbN0_dB | BER_M8_base | BER_M8_kura")
        print("------------------------------------------------")

        for n_lanes in lanes_list:
            for ebn0_db in ebn0_list:
                ber_base, ber_kura = simulate_m8_xn_eq_kuramoto_ber(
                    ebn0_db=ebn0_db,
                    n_sym=args.n_sym,
                    n_lanes=n_lanes,
                    channel_taps=h_b,
    ...
    print("   위상 락 영역에서는 BER_kura가 더 안정적/빠르게 떨어지는지 확인 가능.")
    print(" - drift_std_deg를 키우면 global 위상 random walk가 심해져서,")
    print("   일정 지점을 넘으면 아무리 lane을 늘려도 Kuramoto가 락을 못 거는 한계 영역이 드러난다.")
    print(" - Q13(단일 lane-list, 단일 drift) 결과와 함께 보면,")
    print("   CoPBit 멀티레인 위상 락의 '운영 가능 영역(Operational Region)'을 정의하는 데 쓸 수 있다.")
