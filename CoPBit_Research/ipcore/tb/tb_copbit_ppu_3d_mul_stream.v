// ============================================================
// File: tb/tb_copbit_ppu_3d_mul_stream.v
// Desc: Testbench for copbit_ppu_3d_mul_stream (M=8, NLANES=16)
// ============================================================
`timescale 1ns/1ps

module tb_copbit_ppu_3d_mul_stream;
    ...
    // 테스트 시나리오
    initial begin
        $display("=== CoPBit PPU 3D-MUL stream core (handshake) test (M=%0d, NLANES=%0d) ===", M, NLANES);
        ...
        for (vec = 0; vec < 8; vec = vec + 1) begin
            ...
            for (lane = 0; lane < NLANES; lane = lane + 1) begin
                a_tmp = (vec + lane)   % M;
                b_tmp = (2*vec + lane) % M;
                exp_y_arr[lane] = mul3d_truth(a_tmp, b_tmp);
            ...
            @(posedge clk);
            if (!in_ready) begin
                // 향후 back-pressure TB 확장 고려해서 in_ready 체크
        ...
        if (err_cnt == 0) begin
            $display("[PASS] copbit_ppu_3d_mul_stream (handshake): all vectors OK (M=%0d, NLANES=%0d)", M, NLANES);
        end else begin
            $display("[FAIL] copbit_ppu_3d_mul_stream (handshake): err_cnt=%0d", err_cnt);
        end
